...
int csv_fini(struct csv_parser* p, void (*cb1)(void*, size_t, void*),
    ...
    case FIELD_MIGHT_HAVE_ENDED:
        p->entry_pos -=
            p->spaces + 1; /* get rid of spaces and original quote */
...
size_t csv_parse(struct csv_parser* p, const void* s, size_t len,
    ...
    unsigned const char* us =
        s;           /* Access input data as array of unsigned char */
    ...
    while (pos < len) {
        ...
        case FIELD_NOT_BEGUN:
            if ((is_space ? is_space(c) : c == CSV_SPACE || c == CSV_TAB) &&
                ...
                continue;
                           ...
                           ? is_term(c)
                           : c == CSV_CR || c == CSV_LF) { /* Carriage Return or
                                                              Line Feed */
        ...
        case FIELD_MIGHT_HAVE_ENDED:
            ...
            if (c == delim) { /* Comma */
                entry_pos -=
                    spaces + 1; /* get rid of spaces and original quote */
                SUBMIT_FIELD(p);
            } else if (is_term
                           ? is_term(c)
                           : c == CSV_CR || c == CSV_LF) { /* Carriage Return or
